...
app.get("/posts/:postId", async (req, res) => {
  ...
  try {
    // 2) เขียน Query เพื่ออ่านข้อมูลโพสต์ ด้วย Conection Pool
    ...
    return res.status(500).json({
      message: `Server could not Read post_ID because database connection ${e}`,
    });
  }
...
app.put("/posts/:postId", async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({
        message: `Server could not update a requested post
      (post id: ${postIdFromClient})`,
      });
    }
  } catch (e) {
    return res.status(500).json({
      message: `Server could not update post because database connection ${e}`,
    });
  }
...
app.delete("/posts/:postId", async (req, res) => {
  ...
  try {
    ...
    const value = [postIdFromClient];

    const result = await connectionPool.query(query, value);

    if (result.rowCount === 0) {
      return res.status(404).json({
        message: `Server could not find the requested post to delete (post id: ${postIdFromClient})`,
      });
    }
  } catch (e) {
    return res.status(500).json({
      message: `Server could not delete post because database connection ${e}`,
    });
  }
  return res.status(200).json({
...
app.get("/posts", async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1; // หน้าที่จะแสดง, ค่าเริ่มต้นคือ 1
    const limit = parseInt(req.query.limit) || 6; // จำนวนโพสต์ต่อหน้า, ค่าเริ่มต้นคือ 6
    const offset = (page - 1) * limit; // คำนวณ offset เพื่อใช้ใน query
    const category = req.query.category;
    const keyword = req.query.keyword;
    ...
    if (category && keyword) {
      query +=
        " WHERE category_id = $1 AND (title ILIKE $2 OR description ILIKE $2 OR content ILIKE $2)";
      values = [category, `%${keyword}%`];
    } else if (category) {
    ...
    return res.status(500).json({
      message: `Server could not fetch posts due to a database error: ${e.message}`,
    });
  }
});
*/
